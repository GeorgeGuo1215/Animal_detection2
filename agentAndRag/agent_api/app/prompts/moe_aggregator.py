...
def build_aggregator_prompt(
    *,
    ...
    aggregator = (
        ...
        "payload 中的 evidence_audit 是程序级证据状态：supported 可支撑对应目标；partial 只支撑"
        "matched_hit_ids 对应且已进入 retrieved_sources 的部分；unsupported、unknown、数值门槛失败、"
        "pending_tools、unavailable_required_tools 或必需工具调用失败均表示该目标未核实。未核实时必须"
        "收缩为条件性判断并明确说明无法核实，禁止用模型记忆补齐具体剂量、阈值、洗脱天数、相互作用、"
        "禁忌或现行指南结论。即使专家 conclusion 写得肯定，也必须以本证据状态为准。\n"
        "用户明确要求不提供某个固定数值、天数、剂量或具体处方时，该限制高于专家意见、Critic 和"
        "检索片段；不得用‘经验参考’‘通常建议’或附加免责声明的方式绕过。\n"
