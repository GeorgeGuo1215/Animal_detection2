...
def build_router_system_prompt(expert_description: str, prompt_injection: str = "") -> str:
    prompt = (
        ...
        "宠物身份与照护档案（如名字、物种、品种、年龄、既往史、饮食和照护偏好）是健康随访的必要上下文；"
        "用户要求记住、确认、回忆或更正这些资料时，不属于闲聊，应至少让 clinical 达到可激活分数。"
        "若 history_context.cross_session_memory 非空，说明这是同一用户的跨会话记忆，回忆宠物名或既往病情"
        "属于健康随访续问，不得因当前会话尚无本轮对话而打低分。"
