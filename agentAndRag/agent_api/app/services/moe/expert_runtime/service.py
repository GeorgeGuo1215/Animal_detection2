...
from ....tools.request_scope import ANIMAL_REQUIRED_TOOLS, get_request_animal_id
...
from ....prompts.moe_experts import FORCE_FINAL_REMINDER, build_expert_system_prompt
...
_CHINESE_QUERY_CLAUSE_RE = re.compile(r"[，。；;！？!?]")
_QUERY_REASON_PREFIX_RE = re.compile(
    r"^(?:用户(?:明确)?要求|用户希望|为(?:了)?|需(?:要)?)?"
    r"(?:查询|检索|搜索|查找|核对|确认|联网核实|补充核实)"
)
_SPECIES_QUERY_ALIASES = (
    (re.compile(r"\b(?:canine|dog)\b", re.IGNORECASE), "犬"),
    (re.compile(r"\b(?:feline|cat)\b", re.IGNORECASE), "猫"),
    (re.compile(r"\b(?:equine|horse)\b", re.IGNORECASE), "马"),
    (re.compile(r"\b(?:rabbit|lagomorph)\b", re.IGNORECASE), "兔"),
)


def _compact_chinese_web_query(
    *,
    evidence_goal: str,
    rag_query: str,
    species_zh: Optional[str],
    species_en: Optional[str],
) -> str:
    """把中文证据目标压缩为可检索短语，避免把任务说明整句提交给搜索引擎。"""
    selected = ""
    for raw_clause in _CHINESE_QUERY_CLAUSE_RE.split(str(evidence_goal or "")):
        clause = raw_clause.strip()
        if not clause or not _CHINESE_TEXT_RE.search(clause):
            continue
        clause = _QUERY_REASON_PREFIX_RE.sub("", clause).strip(" ：:，,。；; ")
        if clause:
            selected = clause
            break
    if not selected:
    ...
    species = str(species_zh or "").strip()
    if not species:
        species_source = f"{species_en or ''} {rag_query or ''}"
        for pattern, candidate in _SPECIES_QUERY_ALIASES:
            if pattern.search(species_source):
                species = candidate
                break

    prefixes: List[str] = []
    if species and species not in selected:
        prefixes.append(species)
    if "兽医" not in selected:
        prefixes.append("兽医")
    return " ".join([*prefixes, selected]).strip()[:160]
...
class ExpertAgentSession:
    ...
    def _register_web_fallback_queries(self, result: ToolResult) -> None:
        ...
        chinese_query = _compact_chinese_web_query(
            evidence_goal=evidence_goal,
            rag_query=rag_query,
            species_zh=self.species_zh,
            species_en=self.species_en,
        )
        queries = self.tool_queries.setdefault(WEB_SEARCH_TOOL, [])
        for candidate in (rag_query, chinese_query):
        ...
        for attempt in range(attempts):
            ...
            try:
                ...
                response = await self.llm.chat(
                    messages=messages,
                    temperature=generation.expert_temperature,
