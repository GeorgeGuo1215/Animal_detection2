...
from ....integrations.llm.client import (
    AsyncOpenAIClient,
    AsyncOpenAIStreamClient,
    extract_text,
    get_shared_async_client,
    get_shared_async_stream_client,
)
...
from ..expert_runtime import EXPERTS, ExpertAgentSession, ExpertLoopConfig, run_expert_sessions
...
from ....integrations.petmind_mysql import fetch_animal_profile, species_label
from ....tools.request_scope import get_request_animal_id
...
class MoEOrchestrator:
    ...
    def _build_synthesis_messages(
        ...
        *,
        ...
        # Keep the long system prefix stable for provider prompt caching.  A
        # calendar reference is only meaningful when the task policy actually
        # requested current Web evidence, and stays at the end of the dynamic
        # user payload so ordinary clinical/RAG requests never vary by date.
        if any(task.capability == "current_web" for task in self._active_evidence_tasks):
