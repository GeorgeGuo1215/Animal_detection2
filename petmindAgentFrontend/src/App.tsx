...
import { api, client, resumeRun, setAccessToken, streamRun } from './api'
...
function ChatPage() {
  ...
  useEffect(() => {
    if (!conversationId) { activeConversation.current = ''; setMessages([]); setError(''); return }
  ...
  function handleEvent(event: RunEvent) {
    ...
    if (event.event === 'status') { setError(''); setPhase(String(event.data.phase || '')) }
  ...
  async function recoverRun(runId: string, signal: AbortSignal) {
    ...
    while (!signal.aborted && activeRun.current === runId) {
      ...
      try {
        const snapshot = await api<{status: string; error_message?: string}>(`/api/v1/runs/${runId}`)
        if (['completed', 'failed', 'cancelled'].includes(snapshot.status)) {
          finishRun({ id: lastEvent.current, event: snapshot.status, data: { message: snapshot.error_message || '' } })
          return
        }
  ...
  async function send() {
    ...
    setMessages(prev => [...prev, { id: crypto.randomUUID(), role: 'user', content: text, status: 'complete', created_at: new Date().toISOString() }]); setInput(''); setBusy(true); setError(''); setPhase('queued'); draftAnswer.current = ''; lastEvent.current = 0
