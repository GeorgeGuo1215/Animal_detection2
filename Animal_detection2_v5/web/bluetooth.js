...
(function () {
	...
	class BluetoothManager {
		constructor() {
			...
			// A stable notification id lets the application distinguish the
			// physical BLE notification rate from the number of radar frames
			// carried by each notification.  Date.now() alone is not sufficient:
			// decoding a large multi-frame payload can cross several millisecond
			// ticks and make a 50 Hz frame stream look like 25 Hz.
		...
		async _startNotifications(characteristic = null) {
			...
			try {
				...
				ch.addEventListener('characteristicvaluechanged', (event) => {
					...
					s.count++;
					const notificationId = ++this._notificationSequence;
					...
					// BLE notifications can split a UTF-8 code point across packets.  Keep the
					// decoder state between notifications so a split payload is not corrupted.
		...
		_handleIncomingText(text, notificationMeta = null) {
			...
			while (found) {
				...
				if (delimIdx >= 0) {
					...
					if (line && typeof this.onLine === 'function') {
						...
						try {
							this.onLine(line, notificationMeta ? {
								...notificationMeta,
								frameIndex: lineCount - 1
							} : null);
						} catch (e) {
		...
		async disconnect() {
			try {
				...
				// Do not let an unterminated frame or decoder state leak into the next
				// connection/session.
				try { this.decoder.decode(); } catch (_) {}
				this.decoder = new TextDecoder('utf-8');
				this._rxBuffer = '';
				this.server = null;
				this.device = null;
